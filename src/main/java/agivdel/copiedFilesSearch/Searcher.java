...
public class Searcher {
    ...
    private void iterationFilesFrom(String selectedDirectory) throws IOException {
        ...
        //Вариант №2, с дополнительным классом Doubles(Doubles имеет 2 поля: время и список имен дублей)
    ...
    private void findDuplicates(List<File> fileList) throws IOException {
        ...
        Map<FileTime, List<String>> doubleFiles = fileMap.entrySet()
                .stream()
    ...
    private void findDuplicates2(List<File> fileList) throws IOException {
        ...
        for (File file : fileList) {
            ...
            doublesMap.merge(fileTime, doubles, Doubles::mergeNames);
        }
        doublesMap.values()
                .stream()
                .filter(d -> d.getDoubles().size() > 1)
