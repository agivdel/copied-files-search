...
public class GetDoublesCommand implements Command {
    ...
    private static final Argument<Boolean> ARG_SKIP_ZERO = new Argument<>() {
        ...
        public void convert(final Scanner in, final ArgumentCollector argumentCollector) throws ArgumentCaptureException {
            switch (in.nextLine().toLowerCase()) {
                case "yes":
                    argumentCollector.add(SKIP_ZERO, true);
                    break;
                case "no":
                    argumentCollector.add(SKIP_ZERO, false);
                    break;
                default:
                    throw new ArgumentCaptureException("please answer 'yes' or 'no'");
            }
        }
